...
describe("Path1: Dead Click + Failed Green Bean Checkout", () => {
  it("can add green beans to cart", async () => {
    ...
    await Page.tapSelectorByResourceId(
      "com.fullstorydev.shoppedemo:id/btn_add_to_cart"
    );
  });
  ...
  it("can navigate to checkout screen", async () => {
    ...
    await Page.clickSelectorByResourceId(
      "com.fullstorydev.shoppedemo:id/btn_checkout"
    );
  });

  it("can fill out checkout personal info form", async () => {
    await Page.clickSelectorByResourceId(
      "com.fullstorydev.shoppedemo:id/checkout_first_name"
    );
    await Page.sendKeys("Fruit");

    await Page.clickSelectorByResourceId(
      "com.fullstorydev.shoppedemo:id/checkout_last_name"
    );
    await Page.sendKeys("Buyer");

    await Page.clickSelectorByResourceId(
      "com.fullstorydev.shoppedemo:id/checkout_address_1"
    );
    ...
    await Page.clickSelectorByResourceId(
      "com.fullstorydev.shoppedemo:id/checkout_city"
    );
  ...
  it("can return an error when clicking purchase", async () => {
    await Page.scrollDownUntilResourceIdFound(
      "com.fullstorydev.shoppedemo:id/btn_checkout_purchase"
    );
    ...
    await browser.pause(2000);
    await Page.waitForSelectorByText(
      "invalid security code: should be 3 digits"
    );
  });
});
