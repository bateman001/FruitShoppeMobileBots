...
class Setup {
    ...
    async getFSSessionUrl(buildId, sessionId) {
        ...
        const regexp = /session url: ((https?:\/\/)[^\s]*fullstory[^\s]+(KWH|H0SZW)[^\s]+)/ig;
        const tries = 30;
        let urls;
        let url;
        ...
        for (let i = 1; i <= tries; i++) {
            ...
            try {
                const response = await fetch(`${BROWSERSTACK_APP_AUTOMATE_URL}/builds/${buildId}/sessions/${sessionId}/devicelogs`, {
                    headers: {
                        'Authorization': 'Basic ' + Buffer.from(`${browser.config.user}:${browser.config.key}`).toString('base64'),
