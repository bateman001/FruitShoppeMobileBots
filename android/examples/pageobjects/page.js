...
class Page {
  ...
  async waitForSelectorByText(text, ms) {
    for (let i = 0; i < 10; i++) {
      try {
        const objSelector = await $(`android=new UiSelector().text("${text}")`);
        if (
          (await objSelector.isExisting()) &&
          (await objSelector.isDisplayed())
        ) {
          break;
        }
      } catch (e) {
  ...
  async flingBackward() {
    const objSelector = await $(
      `android=new UiScrollable(new UiSelector().scrollable(true)).flingBackward()`
    );
  }
  ...
  async flingToEnd() {
    const objSelector = await $(
      `android=new UiScrollable(new UiSelector().scrollable(true)).flingToEnd(10)`
    );
  }
